...
class AutoTuner {
public:
    // Start a step-response tune for the selected motor.
    // target: encoder pulses to move (positive or negative).
    // apply: if true, write suggested gains into MotorControl.
    enum Loop { M1, M2 };

    static void start(long target, bool apply, Loop which = M1);
    static void startBoth(long targetM1, long targetM2, bool applyM1, bool applyM2);
    static void abort();
    static void tick();
...
private:
    ...
    static Loop loop;
    static bool chainToM2;
    static long chainTargetM2;
    static bool chainApplyM2;
    struct Sample {
        unsigned long t;
        long target;
        long m1;
